...
const Cart = () => {
	...
	const handleReset = () => {
		...
		if (confirmReset) {
			dispatch(resetCart());
			toast.success('Cart is reset successfully');
			router.push('/');
		}
	}
			...
			(
				<div className="mt-5 flex flex-col">
							...
							{productData.map((item: ProductType) => (
								...
								<tr className="bg-white border-b-[1px] border-b-zinc-300">
									...
									<td className="px-6 py-4 flex items-center gap-4">
											<span
												className="border border-zinc-300 p-1 rounded-md hover:border-zinc-800 cursor-pointer duration-200 inline-flex items-center justify-center">
												<Minus
													onClick={() => item?.quantity > 1 ? dispatch(decreaseQuantity(item)) && toast.success('Quantity decreased successfully') : toast.error('Can not delete less than 1')}
													className="w-4 h-4"/>
											</span>
					...
					<button onClick={handleReset}
					        className="bg-zinc-950 text-zinc-200 w-36 py-3 mt-5 rounded-md uppercase text-xs font-semibold hover:bg-red-700 hover:text-white duration-200">Reset
						Cart
					</button>
					<div className="mt-4 bg-white max-w-xl p-4 flex flex-col gap-1">
						...
						<p className="flex items-center justify-between">
							Price <span><FormattedPrice amount={rowPrice}/></span>
						</p>
						<p className="flex items-center justify-between">
							Discount <span><FormattedPrice amount={rowPrice - totalAmt}/></span>
						</p>
						<p className="flex items-center justify-between">
							Total Price <span><FormattedPrice amount={totalAmt}
							                                  className="font-semibold text-lg"/></span>
						</p>
						<button
